...
int main() {
	...
	par {
		on stdcore[0]:
		{
			...
			int error;
			if(THREADS>7)
				printstr("ERROR: Maximum 7 threads can be used for FIR calculation on one core\nForcing to 7 Threads\n");

			if(THREADS>4)
				printstr("WARNING: More than 4 threads for FIR calculations will result in less taps/s\n");
